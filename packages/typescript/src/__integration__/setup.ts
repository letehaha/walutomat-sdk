...
export function getIntegrationHttp() {
  ...
  if (!privateKeyPath && !privateKeyRaw) {
    throw new Error(
      "WALUTOMAT_PRIVATE_KEY_PATH or WALUTOMAT_PRIVATE_KEY must be set",
    );
  }

  const privateKey = privateKeyRaw ?? readFileSync(privateKeyPath!, "utf-8");

  const sandbox = process.env.WALUTOMAT_SANDBOX === "true";

  return createHttpClient({ apiKey, privateKey, sandbox });
}
