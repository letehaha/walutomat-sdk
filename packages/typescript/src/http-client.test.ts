...
describe("createHttpClient", () => {
  describe("get", () => {
    ...
    it("throws WalutomatHttpError on non-OK response", async () => {
      ...
      await expect(http.get({ endpoint: "/test", signed: false })).rejects.toThrow(
        WalutomatHttpError,
      );
    });
  ...
  describe("post", () => {
    ...
    it("throws WalutomatHttpError on non-OK response", async () => {
      ...
      await expect(
        http.post({ endpoint: "/transfer/internal", body: { volume: "10" }, signed: false }),
      ).rejects.toThrow(WalutomatHttpError);
    });

    it("throws WalutomatApiError when success is false", async () => {
      const fetch = mockFetch({
        success: false,
        errors: [{ key: "INVALID_ACCOUNT", description: "Account not found" }],
      });
      const http = createHttpClient({ apiKey: "k", fetch });
  ...
  describe("postWithDuplicate", () => {
    ...
    it("throws WalutomatHttpError on non-OK response", async () => {
      const fetch = mockFetch({ error: "forbidden" }, 403);
      const http = createHttpClient({ apiKey: "k", fetch });

      await expect(
        http.postWithDuplicate({ endpoint: "/direct_fx/exchanges", body: {}, signed: false }),
      ).rejects.toThrow(WalutomatHttpError);
    });
    ...
    it("throws WalutomatApiError when success is false", async () => {
      const fetch = mockFetch({
        success: false,
        errors: [{ key: "EXPIRED_RATE", description: "Rate has expired" }],
      });
      const http = createHttpClient({ apiKey: "k", fetch });
